...
def prepare_4C_loc(study_dir, save_dirs, width, verbose=True):
    # first dig down to the root directory of data
    dcm_dir, sid, pid, date = trace_to_dcm_dir(study_dir)

    save_name = sid + '-[' + pid + ']-[' + date + ']'

    # process 4C localizer
    info_loc, img_loc = process_LAX_localizer(dcm_dir, 'trufi_4-chamber', -1, save_dirs['loc'], save_name + '-[IMG]',
                                              verbose=verbose)
    ...
    for j, alpha in enumerate(width):
        heatmap = np.concatenate((heatmap_2C[j][None, ...], heatmap_3C[j][None, ...], heatmap_SA[j][None, ...]), axis=0)
...
if __name__ == '__main__':
    ...
    save_dirs = {'loc': r'..\data\view_plan_data_4C_loc',
    ...
    for study_dir in exam_dirs:
        ...
        prepare_4C_loc(study_dir, save_dirs, width, True)
